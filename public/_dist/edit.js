...
var textArea = document.getElementById('query'); // eslint-disable-line no-unused-vars


var QueryModule = function (_Domodule) {
  inherits(QueryModule, _Domodule);

  function QueryModule(el) {
    classCallCheck(this, QueryModule);

    var _this = possibleConstructorReturn(this, (QueryModule.__proto__ || Object.getPrototypeOf(QueryModule)).call(this, el));

    _this.setupDbs();
    _this.editor = _this.setupEditor();
    return _this;
  }

  createClass(QueryModule, [{
    key: 'setupDbs',
    value: function setupDbs() {
      var dbSelect = this.els.databases;
      Ajax.request('/api/databases', 'GET', null, function (err, resp) {
        if (err) {
    ...
    key: 'getCollections',
    value: function getCollections(el) {
      var db = el.value;
      var collSelect = this.els.collections;
      collSelect.innerHTML = '<option>Collection</option>';
      collSelect.disabled = true;
      Ajax.request('/api/databases/' + db + '/collections', 'GET', null, function (err, resp) {
        if (err) {
          alert(err); // eslint-disable-line no-alert
        }
    ...
    value: function submitQuery() {
      ...
      Ajax.request('/api/query', 'POST', data, function (err, resp) {
        if (err) {
          alert(err); // eslint-disable-line no-alert
        }

        if (resp.data) {
          var respData = JSON.stringify(resp.data, null, 2);
          var html = prism.highlight(respData, prism.languages.json);
          _this2.els.response.innerHTML = '<pre class="language-json">' + html + '</pre>';
        }
      });
    }
  }, {
    key: 'doQuery',
    value: function doQuery(el, event) {
